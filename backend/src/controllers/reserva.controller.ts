...
export const ReservaController = {
    crear: async (req: Request, res: Response) => {
        try {
            ...
            if (rolNormalizado === 'empleado' && !id_cliente) {
                return res.status(400).json({ error: 'Debe seleccionar un cliente para la reserva presencial' });
            }
    ...
    cancelar: async (req: Request, res: Response) => {
        try {
            ...
            const { rol, id_usuario } = (req as any).usuario;
            
            const resultado = await ReservaService.cancelarReserva(Number(id), motivo, rol);
            
            res.json({ 
                success: true, 
                data: resultado,
                message: resultado.reembolso 
                    ? `Reserva cancelada. Política de reembolso: ${resultado.reembolso.politica_aplicada}. Monto a reembolsar: Bs. ${resultado.reembolso.monto_reembolsado}`
                    : 'Reserva cancelada'
            });
        } catch (error: any) {
